                        ...
                        <div class="modal-content"> 
                            <form method="POST" id="editEmployeeForm">
                                ...
                                <div class="modal-header">
                                    <h3>Thông tin</h3>
                                    <input type="hidden" name="salary_id" id="salaryId"> <!-- Đặt id cho input -->
    ...
    <script>
        $(document).ready(function() {
            $('.edit').click(function() {
                ...
                $('#salaryId').val(id); // Sử dụng salaryId thay cho id
                $('#name').text(name);
                $('#email').text(email);
                $('#month').text(month);
                ...
                $('#salary').text(salary);
                $('#status').val(status);
                $('#hiddenStatus').val(status); // Cập nhật hiddenStatus
                // $('#statusDropdown').val(status); // Cập nhật giá trị dropdown
            });

            $('#editEmployeeForm').submit(function(event) {
                event.preventDefault();
                var id = $('#salaryId').val(); // Sử dụng salaryId thay cho id
                var formData = {
