...
<div class="container">
  <div class="table-wrapper h-200 shadow-lg mt-8">
      ...
      <tr>
        ...
        <td><a href="#salaryPayment" class="edit" data-bs-toggle="modal" data-status="{{ $userScan->status }}" data-id="{{ $userScan->user_id }}" data-name="{{ $userScan->name }}" data-month="{{ $userScan->month }}" data-email="{{ $userScan->email }}" data-salary="{{ $userScan->salary_amount }}">
  ...
  <script>
    $(document).ready(function() {
        $('.edit').click(function() {
            ...
            $('#name').val(name);
            $('#email').val(email);
            $('#work_hour').val(work_hour); // Sửa #username thành #work_hour
            $('#salary').val(salary);
        }); // Đóng hàm click
    }); // Đóng hàm ready
    </script>
