        ...
        if ($checkIn) {
            if(!$checkIn->check_out){
                ...
                if(Salary::where('user_id', $user->id)){
                    $startTime = Carbon::parse($user->check_in); 
                 $endTime = Carbon::parse($user->check_out);
                    Salary::where('user_id', $user->id)->increment('work_hours', $user->salary);
                    Salary::where('user_id', $user->id)->increment('salary_amount', $user->salary);
                }
