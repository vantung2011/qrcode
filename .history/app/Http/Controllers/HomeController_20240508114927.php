    ...
    public function store(Request $request)
    {
        ...
        if ($checkIn) {
            if(!$checkIn->check_out){
                $diffInSeconds = Carbon::now()->diffInSeconds($user->check_in);
                $minimumScanInterval = 5;
                if (Carbon::now()->diffInSeconds($user->check_in)<2) {
                ...
                Salary::create(
                    ['user_id' => $user->id, 'month' => Carbon::now()->format('Y-m'),
                    'work_hours' => $workHours, 'salary_amount' => $totalSalary]
                );
