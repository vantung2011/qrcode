...
class HomeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($checkIn) {
            $timeDifferenceMinutes = Carbon::now()->diffInMinutes($checkIn->check_in);
            $timeDifferenceMinutes1 = Carbon::parse($timeDifferenceMinutes);
            $minimumScanInterval = 1;
            if(!$checkIn->check_out && $timeDifferenceMinutes1 > $minimumScanInterval) {
