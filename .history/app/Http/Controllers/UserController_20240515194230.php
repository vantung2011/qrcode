...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $users = User::join('work_shifts', 'users.work_shift_id', '=', 'work_shifts.id')
             ->select('users.*', 'work_shifts.name as work_shift_name')
             ->paginate(5)
             ->withQueryString();
        
        if ($request->search) {
            $search = $request->search;
            $users = User::where('name', 'like', "%$search%")->orWhere('email', 'like', "%$search%")->with('workShift')->paginate(5)->withQueryString();
            if ($users->count() >= 1) {
